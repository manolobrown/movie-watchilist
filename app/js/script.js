            ...
            htmlContent.innerHTML += `<div class="card">
              ...
              <div class="info">
                ...
                <div class="meta">
                  ...
                  <button data-imdbid="${data.imdbID}">
                    <div class="icon">
                      <svg
                        width="16"
                        height="16"
...
function removeFromWatchList(id) {
  ...
  const filmIDs = films.filter((val) => val != id);
...
function getSavedFilms() {
  ...
  const films = savedFilms ? JSON.parse(savedFilms) : [];

  if (!films.length)
    return (watchList.innerHTML = `<h2 class="heading-start">
            Your watchlist is lookinga little empty...
            <a href="/"
              ><span class="icon"
                ><svg
                  width="16"
      ...
      watchList.innerHTML += `<div class="card">
              ...
              <div class="info">
                ...
                <div class="meta">
                  <div class="run-time">${data.Runtime}</div>
                  <div class="genre">${data.Genre}</div>
                  <button data-remove-imdbid="${data.imdbID}">
                    <div class="icon">
...
document.addEventListener("click", (e) => {
  ...
  let imdbButton = e.target;
  if (!imdbID) return;
  imdbButton.innerText = "Added to List!";
  imdbButton.disabled = true;
  saveToWatchList(imdbID);
});

document.addEventListener("click", (e) => {
  let imdbID = e.target.dataset.removeImdbid;
  if (!imdbID) return;
